...
export class LocalBrainEngine {
  ...
  async executeBrainAction(action, context, policy, response) {
    ...
    const executableAction = {
      ...
      args: {
        ...(action.args ?? {}),
        // The LLM never supplies this. Only a current typed/voice user event
        // can authorize a gimbal action through the tool executor.
        userInitiated: context.triggerEvent?.type === "user_text"
      },
