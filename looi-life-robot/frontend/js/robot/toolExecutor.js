...
export class ToolExecutor {
  ...
  setGimbalModeHandler(setGimbalMode) {
    this.setGimbalMode = setGimbalMode;
  }

  // Extension point for explicitly registered, browser-owned voice actions.
  // Model declarations and local-brain validation must still whitelist the type.
  registerVoiceCommand(type, handler, { physical = false } = {}) {
    const normalizedType = String(type ?? "").trim();
    if (!normalizedType || BUILT_IN_ACTION_TYPES.has(normalizedType)) {
      throw new Error(`Voice command type is reserved or invalid: ${normalizedType || "missing"}`);
    }
    if (typeof handler !== "function") {
      throw new Error(`Voice command handler must be a function: ${normalizedType}`);
    }

    this.voiceCommandHandlers.set(normalizedType, { handler, physical: Boolean(physical) });
    return () => this.voiceCommandHandlers.delete(normalizedType);
  }
  ...
  async executeTool(type, args = {}, action = {}) {
    switch (type) {
      ...
      default:
        return this.executeRegisteredVoiceCommand(type, args, action);
    }
  }
  ...
  async executeSetGimbalMode(args = {}, action = {}) {
    const mode = normalizeGimbalMode(args.mode ?? args.gimbalMode);
    ...
    if (!mode) {
      return this.buildResult("rejected", {
        ...
        executed: false,
        physical: false,
        message: "set_gimbal_mode requires curious_idle or off."
      });
    }
    ...
    if (args.userInitiated !== true) {
      return this.buildResult("rejected", {
        ...
        executed: false,
        physical: false,
        message: "Gimbal mode changes require a direct user command."
      });
    }
    ...
    if (!this.isAutonomousControlEnabled()) {
      return this.buildResult("rejected", {
        ...
        executed: false,
        physical: false,
        message: "Gimbal mode changes are disabled while manual control is active."
      });
    }

    if (typeof this.setGimbalMode !== "function") {
      return this.buildResult("failed", {
        action,
        executed: false,
        physical: false,
        message: "Gimbal mode controller is unavailable."
      });
    }

    const reason = normalizeShortText(args.reason ?? action.reason, 160)
      || "set_gimbal_mode";

    try {
      const outcome = await this.setGimbalMode({
        mode,
        source: action.source ?? "tool_executor",
        reason,
        userInitiated: true
      });
      const accepted = outcome?.ok !== false;

      return this.buildResult(accepted ? "completed" : "rejected", {
        action,
        executed: accepted,
        physical: false,
        message: outcome?.message ?? `Gimbal mode set to ${mode}.`,
        detail: {
          mode,
          reason,
          ...(outcome?.detail && typeof outcome.detail === "object"
            ? { controller: outcome.detail }
            : {})
        }
      });
    } catch (error) {
      this.log(`Gimbal mode change failed: ${error.message}`, "warn");
      return this.buildResult("failed", {
        action,
        executed: false,
        physical: false,
        message: `Gimbal mode change failed: ${error.message}`
      });
    }
  }

  async executeMoveGimbal(args = {}, action = {}) {
    const direction = normalizeGimbalDirection(args.direction);
    if (!direction) {
      return this.buildResult("rejected", {
        action,
        executed: false,
        physical: true,
        message: "move_gimbal requires left, right, up, down, or center."
      });
    }
    ...
    if (args.userInitiated !== true) {
      return this.buildResult("rejected", {
        ...
        executed: false,
        physical: true,
        message: "Gimbal movement requires a direct user command."
      });
    }
    ...
    if (!this.isAutonomousControlEnabled()) {
      return this.buildResult("rejected", {
        ...
        executed: false,
        physical: true,
        message: "Gimbal movement is disabled while manual control is active."
      });
    }

    if (typeof this.moveGimbal !== "function") {
      return this.buildResult("failed", {
        action,
        executed: false,
        physical: true,
        message: "Gimbal movement controller is unavailable."
      });
    }

    try {
      const outcome = await this.moveGimbal({
        direction,
        degrees: args.degrees,
        durationMs: args.durationMs,
        source: action.source ?? "tool_executor",
        reason: normalizeShortText(args.reason ?? action.reason, 160) || "move_gimbal",
        userInitiated: true
      });
      const accepted = outcome?.ok !== false;
      return this.buildResult(accepted ? "completed" : "rejected", {
        ...
        executed: accepted,
        physical: true,
        message: outcome?.message ?? `Gimbal moved ${direction}.`,
        detail: { direction, ...(outcome?.detail ?? {}) }
      });
    } catch (error) {
      this.log(`Gimbal movement failed: ${error.message}`, "warn");
      return this.buildResult("failed", {
        action,
        executed: false,
        physical: true,
        message: `Gimbal movement failed: ${error.message}`
      });
    }
  }
  ...
  async executeRegisteredVoiceCommand(type, args = {}, action = {}) {
    ...
    try {
      const outcome = await registered.handler({
        args,
        action,
        runtimeContext: this.getRuntimeContext?.() ?? null
      });
      const accepted = outcome?.ok !== false;
      return this.buildResult(accepted ? outcome?.status ?? "completed" : "rejected", {
        action,
        executed: accepted && outcome?.executed !== false,
        physical: outcome?.physical ?? registered.physical,
        message: outcome?.message ?? `Voice command completed: ${type}`,
        detail: outcome?.detail ?? {}
      });
    } catch (error) {
      this.log(`Registered voice command failed (${type}): ${error.message}`, "warn");
      return this.buildResult("failed", {
        action,
        executed: false,
        physical: registered.physical,
        message: `Voice command failed: ${error.message}`
      });
    }
  }
  ...
  validateActionShape(action) {
    ...
    if (typeof action.type !== "string" || !this.isActionTypeAllowed(action.type)) {
  ...
  isPhysicalAction(type) {
    return PHYSICAL_ACTIONS.has(type) || Boolean(this.voiceCommandHandlers.get(type)?.physical);
  }

  isActionTypeAllowed(type) {
    return BUILT_IN_ACTION_TYPES.has(type) || this.voiceCommandHandlers.has(type);
  }
  ...
  isMotionArmed(action = {}) {
    const policy = this.policy();
    return Boolean(policy.localMotionArmed) && this.isAutonomousControlEnabled(policy);
  }

  isAutonomousControlEnabled(policy = this.policy()) {
    return policy.controlMode !== "manual";
  }
