...
import { getApi2dChatEnv } from "../api2d/api2dChatEnv.js";
...
export function createLocalBrainServerFromEnv(env = process.env, logger) {
  ...
  const api2d = getApi2dChatEnv(env);
  ...
  try {
    return new LocalBrainServer({
      ...
      provider: createProvider(providerName, {
        ...
        openAiApiKey: env.OPENAI_API_KEY || env.LOCAL_BRAIN_OPENAI_API_KEY || "",
        openAiResponsesBaseUrl:
          env.OPENAI_BASE_URL ||
          env.LOCAL_BRAIN_OPENAI_BASE_URL ||
...
function createProvider(providerName, options) {
  switch (providerName) {
    ...
    case "api2d":
      return new OpenAICompatibleProvider({
        baseUrl: options.api2d.baseUrl,
        apiKey: options.api2d.apiKey,
        model: options.model ?? options.api2d.model,
        timeoutMs: options.timeoutMs,
        temperature: options.temperature,
        maxOutputTokens: options.maxOutputTokens,
        name: "api2d",
    ...
    case "openai-responses":
      return new OpenAIResponsesProvider({
        baseUrl: options.openAiResponsesBaseUrl,
...
function normalizeProvider(value) {
  ...
  return ["mock", "rule", "ollama", "groq", "fireworks", "openai-compatible", "openai-responses", "api2d"].includes(provider)
    ? provider
    : "mock";
}
