...
const gimbalModes = [];
executor.setGimbalModeHandler(async ({ mode, source, reason }) => {
  gimbalModes.push({ mode, source, reason });
  return {
    ok: true,
    message: `Gimbal mode set to ${mode}.`,
    detail: { mode }
  };
});
const gimbalModeResult = await executor.executeAction({
  id: "gimbal_curious_idle",
  source: "gemini_live",
  type: "set_gimbal_mode",
  args: { mode: "curious_idle", reason: "voice_request", userInitiated: true }
});
assert.equal(gimbalModeResult.status, "completed");
assert.equal(gimbalModeResult.executed, true);
assert.equal(gimbalModeResult.physical, false);
assert.deepEqual(gimbalModes.at(-1), {
  mode: "curious_idle",
  source: "gemini_live",
  reason: "voice_request"
});
const invalidGimbalModeResult = await executor.executeAction({
  id: "gimbal_invalid",
  source: "gemini_live",
  type: "set_gimbal_mode",
  args: { mode: "raw_pwm" }
});
assert.equal(invalidGimbalModeResult.status, "rejected");
const ambientGimbalModeResult = await executor.executeAction({
  id: "gimbal_ambient_mode",
  source: "gemini_live",
  type: "set_gimbal_mode",
  args: { mode: "curious_idle" }
});
assert.equal(ambientGimbalModeResult.status, "rejected");

const manualPolicyExecutor = new ToolExecutor({
  robotClient,
  commandQueue,
  getExecutionPolicy: () => ({ localMotionArmed: false, controlMode: "manual" }),
  setGimbalMode: async () => ({ ok: true }),
  moveGimbal: async () => ({ ok: true })
});
const manualModeBlocked = await manualPolicyExecutor.executeAction({
  id: "manual_mode_gimbal_blocked",
  source: "gemini_live",
  type: "set_gimbal_mode",
  args: { mode: "curious_idle", userInitiated: true }
});
assert.equal(manualModeBlocked.status, "rejected");
assert.match(manualModeBlocked.message, /manual control/i);

const gimbalMoves = [];
executor.moveGimbal = async ({ direction, degrees, durationMs, source, reason }) => {
  gimbalMoves.push({ direction, degrees, durationMs, source, reason });
  return { ok: true, message: `Gimbal moved ${direction}.` };
};
const gimbalMoveResult = await executor.executeAction({
  id: "gimbal_left",
  source: "gemini_live",
  type: "move_gimbal",
  args: { direction: "left", degrees: 20, reason: "voice_request", userInitiated: true }
});
assert.equal(gimbalMoveResult.status, "completed");
assert.equal(gimbalMoveResult.executed, true);
assert.equal(gimbalMoveResult.physical, true);
assert.deepEqual(gimbalMoves.at(-1), {
  direction: "left",
  degrees: 20,
...
const invalidGimbalMoveResult = await executor.executeAction({
  id: "gimbal_invalid_direction",
  source: "gemini_live",
  type: "move_gimbal",
  args: { direction: "raw_pwm" }
});
assert.equal(invalidGimbalMoveResult.status, "rejected");
const ambientGimbalMoveResult = await executor.executeAction({
  id: "gimbal_ambient_move",
  source: "gemini_live",
  type: "move_gimbal",
  args: { direction: "up" }
});
assert.equal(ambientGimbalMoveResult.status, "rejected");
