...

{
  ...
  assert.equal(setResult.status, "rejected");
  assert.match(setResult.message, /disabled/i);
  const geminiStop = await executor.executeAction({
    type: "run_scenario",
    source: "gemini_live",
    args: { name: "stop_following", reason: "conversation_turn" }
  });
  assert.equal(geminiStop.status, "rejected");
  assert.match(geminiStop.message, /disabled/i);
  ...
  assert.match(stopResult.message, /disabled/i);
}

{
  assert.deepEqual([...LOCAL_BRAIN_ALLOWED_ACTIONS].sort(), ["move_gimbal", "run_scenario", "set_gimbal_mode"]);
  assert.equal(validateBrainAction({ type: "run_scenario", args: { name: "follow_target", label: "apple" } }).ok, false);
